...
def payments_user(for_user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Я оплатил", callback_data=f"payments_approve_{for_user_id}"),
            InlineKeyboardButton(text="❌ Ошибка", callback_data=f"payments_decline_{for_user_id}")
        ]
    ])

def payments_admin(for_user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Оплата прошла", callback_data=f"admin_approve_{for_user_id}")
        ]
    ])
...
def ensure_user_row(user_id: int):
    cur.execute("INSERT OR IGNORE INTO users(user_id, approved) VALUES(?, 1)", (user_id,))
    conn.commit()
...
def is_approved(user_id: int) -> int:
    ...
    row = cur.fetchone()
    return int(row[0]) if row else 1  # По умолчанию одобрен
...
async def payments_approve(cb: CallbackQuery):
    await cb.answer()
    await cb.message.answer(
        "Платеж на проверке, в скором времени выдадим конфиг.\n"
        "Приблизительное время ожидания - 30 минут"
    )
    ...
    user = cb.from_user
    message_text = (
        f"🔄 Новый платеж на проверке!\n"
        f"👤 Пользователь: {user.full_name} (@{user.username})\n"
        f"🆔 ID: {user.id}\n"
        f"⏰ Время: {cb.message.date.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    
    # Отправляем сообщение администратору с кнопкой для подтверждения
    await bot.send_message(
        ADMIN_ID, 
        message_text,
        reply_markup=payments_admin(user_id)
    )
...
async def admin_approve(cb: CallbackQuery):
    ...
    try:
        ...
        create_config_record(user_id, user_uuid, days=30, is_trial=False, created_by_admin=False)
    except Exception as e:
        await cb.message.answer(f"❌ Ошибка при создании конфига: <code>{e}</code>")
        return
    ...
    try:
        await bot.send_message(
            user_id, 
            "✅ Ваш платеж подтвержден! Вот ваш конфиг:\n"
            f"<code>{link}</code>"
        )
    except Exception as e:
    ...
    await bot.send_message(ADMIN_ID, "✅ Вы подтвердили оплату пользователя!")
    
    # Редактируем сообщение с кнопкой
    await cb.message.edit_text("✅ Оплата подтверждена администратором")
